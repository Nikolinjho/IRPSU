...
export default {
  ...
  methods: {
    ...
    removeLetter() {
      ...
      //   letter.locked = false
      //   const prevLetter = this.letters.find(item => item.id === this.lastLetter.link);
      //   console.log("🚀 ~ file: Game.vue ~ line 90 ~ removeLetter ~ prevLetter", prevLetter)
      //   if (prevLetter){
      //     this.lastLetter = prevLetter
      //   }
      // }
    },
    pressKey(letter) {
      if (!letter.locked){
        ...
        // this.lastLetter = {...letter, link: this.lastLetter.id}
      }
      const compared = this.compareByLetter()
      if (compared){
        const pressedLetter = this.pat.find(item => item.id === letter.id)
    ...
    async getData() {
      const randomId = 1332// Math.floor(
